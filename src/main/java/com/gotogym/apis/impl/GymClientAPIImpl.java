...
public class GymClientAPIImpl implements GenericAPI {
	...
	private boolean validateGymClientInput(GymClient gymClient) {
		if (gymClient != null && gymClient.getClientName() != null && gymClient.getPhone() != null
				&& gymClient.getEmail() != null && gymClient.getCity() != null && gymClient.getSalt() != null
				&& gymClient.getHash() != null && gymClient.getAddress1() != null) {
			return true;
		}
		return false;
	}
	...
	@Path("/updatePass")
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateClientPassword(@QueryParam("salt") String passSalt, @QueryParam("hash") String passHash,
			@QueryParam("phone") Long phone) {
		try {
			if (passSalt != null && !passSalt.trim().isEmpty() && passHash != null && !passHash.trim().isEmpty()
					&& phone != null) {

				passSalt = passSalt.trim();
				passHash = passHash.trim();
				GymClientDAOImpl.getObject().updateClientPassword(passSalt, passHash, phone);
